...
func (r *auditRepository) AppendWithTip(ctx context.Context, event *AuditEvent, tip string) error {
	if event == nil {
		return fmt.Errorf("append audit event with tip: event is nil")
	}
	if event.Action == "" {
		event.Action = event.EventType
	}
	if event.Action == "" {
		return fmt.Errorf("append audit event with tip: action is required")
	}
	if event.EventType == "" {
		event.EventType = event.Action
	}
	...
	if _, err := tx.ExecContext(ctx, `
		...
		VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, event.ID, event.EventType, event.Actor, event.Metadata, event.Action, event.TargetType, event.TargetID, event.Result, event.DetailsJSON, event.PrevHash, event.EventHash, fmtTime(event.CreatedAt)); err != nil {
		return fmt.Errorf("append audit event with tip: insert: %w", err)
	}

	if _, err := tx.ExecContext(ctx, `INSERT OR REPLACE INTO vault_meta(key, value) VALUES(?, ?)`, auditChainTipMetaKey, tip); err != nil {
		return fmt.Errorf("append audit event with tip: set chain tip: %w", err)
	}

	if err := tx.Commit(); err != nil {
		return fmt.Errorf("append audit event with tip: commit: %w", err)
	}
	return nil
}
